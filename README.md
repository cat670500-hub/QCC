...
* **🔄 即時 WebSocket 代理端與雙通道 Fallback 機制 (Proxy Agent & Dual-Channel Fallback)**
  * **即時代理報到**：當其他端（如手機、其他電腦網頁）點擊報到或取消報到時，主伺服器將即時以 WebSocket 轉發指令至具備醫院內網 VPN 權限的本機代理端執行，並在成功後立即重新撈取最新名單推播刷新，完成「秒級更新」。
  * **雙通道防護網**：當網路異常或 Socket 離線時，系統自動 Fallback 回 10 秒輪詢的 API 佇列任務發送機制（`/api/pending_check_ins`），已修正免登入過濾，確保流程百分之百不中斷。

* **⚠️ 雙重錯誤追蹤日誌與卡片即時警示 (Dual-Error Logs & Card-Level Alerts)**
  * **病患卡片即時錯誤反饋**：若本機代為報到失敗，系統會即時在該病患卡片上渲染 `⚠️ 同步失敗: [原因]` 的紅色警告，並跳出詳細的對話框提醒。
  * **三欄式日誌控制台**：發送端面板底部配備三欄式實時日誌監控面版，實現即時的 Ajax 刷新加載：
    * 醫院同步失敗日誌（`sync_errors.log`，紀錄 Web Socket 回報錯誤）
    * 本機報到失敗日誌（`check_in_errors.log`，紀錄本地直連醫院 API 失敗）
    * 確認紀錄清單（正常的派遣確認歷史）
